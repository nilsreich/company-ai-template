...
namespace Examples\InvoiceExtraction\Console;
...
class EvaluateInvoice extends Command
{
    ...
    public function handle(): int
    {
        ...
        $extractor = $driver === 'fake' ? app(FakeTaskExtractor::class) : app(LiveTaskExtractor::class);
        ...
        $counts = array_fill_keys(InvoiceExtraction::FIELDS, 0);
        $complete = 0;
        $failures = 0;
        foreach ($manifests as $path) {
            try {
                ...
                $expected = app(InvoiceValidator::class)->handle($fixture['fields'] ?? null);
                $result = $extractor->extract(new TaskInput($pdf, 1, $model, $prompt, mimeType: 'application/pdf'));
                $actual = app(InvoiceValidator::class)->handle($result->payload);
                $matched = 0;
                ...
                $complete += $matched === count(InvoiceExtraction::FIELDS) ? 1 : 0;
            } catch (\Throwable) {
                $failures++;
                $this->warn('Fixture nicht auswertbar: '.basename(dirname($path)).' (keine Inhaltsausgabe).');
            }
        }
        $accuracy = array_sum($counts) / (count($manifests) * count(InvoiceExtraction::FIELDS));
        ...
        $this->table(['Feld', 'Korrekt', 'Fälle'], array_map(fn (string $field): array => [$field, $counts[$field], count($manifests)], InvoiceExtraction::FIELDS));
